...
def test_register_success(client):
    """Register a new user."""
    resp = client.post(
        "/api/v1/auth/register", json={"email": "user@example.com", "password": "securepass123"}
    )
...
def test_register_duplicate_email(client):
    ...
    resp = client.post(
        "/api/v1/auth/register", json={"email": "dup@example.com", "password": "other12345"}
    )
...
def test_login_success(client):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "login@example.com", "password": "mypass123"}
    )
    resp = client.post(
        "/api/v1/auth/login", json={"email": "login@example.com", "password": "mypass123"}
    )
...
def test_refresh_token(client):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "refresh@example.com", "password": "mypass123"}
    )
    login_resp = client.post(
        "/api/v1/auth/login", json={"email": "refresh@example.com", "password": "mypass123"}
    )
    ...
    assert "access_token" in data
    assert "refresh_token" in data
    # New access token can be used for protected route
    me_resp = client.get(
...
def test_login_invalid_password(client):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "wrong@example.com", "password": "correct123"}
    )
    resp = client.post(
...
def test_login_nonexistent_user(client):
    ...
    resp = client.post(
        "/api/v1/auth/login", json={"email": "nobody@example.com", "password": "anypass12"}
    )
    assert resp.status_code == 401
